...
def run_model(df_raw: pd.DataFrame) -> dict:
    ...
    # ── SEL 3: Interpolasi & Seasonal Decompose ──────────────────────────────
    ...
    reconstructed = (dec.trend + dec.seasonal).interpolate(method='time').bfill().ffill()
    ...
    # ── SEL 6: Differencing d=1 ───────────────────────────────────────────────
    series_filled = data['kunjungan_wisatawan_filled']
    diff_1 = series_filled.diff().dropna()

    # ── SEL 7: Differencing musiman s=12 ──────────────────────────────────────
    diff_seasonal = series_filled.diff(12).dropna()

    # ── SEL 8: ADF diff musiman ───────────────────────────────────────────────
    adfuller(diff_seasonal)
    ...
    if len(test_data) < 2:
        raise ValueError("Data terlalu sedikit. Minimal sekitar 20 baris data bulanan.")
    ...
    look_back              = min(6, max(1, len(residual_test) - 2))
    X_test_res, y_test_res = create_lstm_dataset(residual_test.values, look_back)

    if len(X_test_res) == 0:
        raise ValueError("Data testing terlalu sedikit untuk LSTM.")

    X_test_res = X_test_res.reshape((X_test_res.shape[0], X_test_res.shape[1], 1))

    # ── SEL 17: Train LSTM final (parameter terbaik dari notebook) ───────────
    K.clear_session()
    ...
    model_lstm.fit(
        ...
        epochs=100,
        batch_size=min(46, len(X_test_res)),
        verbose=0
    )

    # ── SEL 18: Prediksi residual ─────────────────────────────────────────────
    ...
    def normalize_index(s):
        sc = s.copy()
        sc.index = pd.DatetimeIndex(sc.index.to_period('M').to_timestamp())
        if sc.index.duplicated().any():
            sc = sc.groupby(sc.index).mean()
        return sc
    ...
    residual_test_clean_n = normalize_index(residual_test)
    ...
    seed_window          = [float(v) for v in residual_test_clean_n.values[-look_back:]]
    ...
    return {
        "historis":      {"dates": fmt(hist_only.iloc[-12:].index), "values": hist_only.iloc[-12:].values.tolist()},
        "testing":       {"dates": fmt(hybrid_test_asli.index),     "values": hybrid_test_asli.values.tolist()},
        "sarima_future": {"dates": fmt(future_index),               "values": [round(float(v), 2) for v in sarima_future_asli]},
        "hybrid_future": {"dates": fmt(future_index),               "values": [round(float(v), 2) for v in hybrid_future_asli]},
        "metrics": {
            "mae":  round(float(mae_hybrid),  2),
            "mse":  round(float(mse_hybrid),  2),
            "rmse": round(float(rmse_hybrid), 2),
            "mape": round(float(mape_hybrid), 4),
        },
